...
export default function ResultsPanel({
	...
	const filteredBusinesses = hideDuplicates
		? businessesWithDuplicateDetection.filter(
				(business) => !business.isDuplicate
		  )
	...
	const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		reader.onload = async (event) => {
			try {
				...
				if (csvData) {
					const result = await importBusinessesMutation.mutateAsync(
						csvData
					);
					toast({
													...
													onClick={async () => {
														try {
															...
															toast({
																...
																description:
																	'All search results have been removed.',
															});
														} catch (error) {
															toast({
																title: 'Error',
																description:
																	'Failed to clear search results. Please try again.',
																variant:
																	'destructive',
															});
														}
						...
						<div className='flex flex-col sm:flex-row sm:justify-between sm:items-center pt-4 border-t border-gray-100'>
							<div>
								...
								<p className='text-xs text-gray-500'>
									Add search results to your personal company
									list
								</p>
							</div>
									...
									onClick={async () => {
										try {
											...
											toast({
												title: 'Error',
												description:
													'Failed to add companies to your list',
												variant: 'destructive',
											});
										}
									}}
					...
					<CardContent className='p-8 text-center'>
						<div className='flex flex-col items-center'>
							...
							<p className='text-sm text-gray-500 max-w-md'>
								Searching for businesses matching your criteria.
				...
				<Card className='border-l-4 border-[hsl(var(--destructive))]'>
					<CardContent className='p-6'>
						<div className='flex'>
							...
							<div className='ml-3'>
								<h3 className='text-sm font-medium text-[hsl(var(--destructive))]'>
